...
- **Portfolio previews** — each tile shows a **live preview of the hosted 360° tour**, embedded straight from its URL (lazy-loaded, non-interactive; the tile clicks through to the full tour). Nothing to upload. To feature a different tour, change both the tile's `<iframe class="work__tour" src="…">` and its `<a class="work__link" href="…">`. See [`assets/tour-previews.md`](./assets/tour-previews.md) for how the fallback works and how to add a static image for any tour that blocks embedding.
